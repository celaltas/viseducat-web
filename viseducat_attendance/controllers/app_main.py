...
class VmAttendanceController(http.Controller):

    @http.route(['/viseducat-attendance/take-attendance'], type='json',
                auth='none', methods=['POST'], csrf=False)
    def create_attendance_lines(self, **post):
        sheet_id = post.get('attendance_sheet_id', False)
        if sheet_id:
            attend_lines = request.env['vm.attendance.line'].sudo()
            sheet = request.env['vm.attendance.sheet'].sudo().browse(
                [sheet_id])
            all_student_search = request.env['vm.student'].sudo().search(
