...
class VmParent(models.Model):
    ...
    student_ids = fields.Many2many('vm.student', string='Student(s)')
    ...
    def create(self, vals):
        res = super(VmParent, self).create(vals)
    ...
    def create_parent_user(self):
        template = self.env.ref('viseducat_parent.parent_template_user')
...
class VmStudent(models.Model):
    ...
    def get_parent(self):
        action = self.env.ref('viseducat_parent.'
                              'act_open_vm_parent_view').read()[0]
        action['domain'] = [('student_ids', 'in', self.ids)]
        return action


class VmSubjectRegistration(models.Model):
    _inherit = "vm.subject.registration"
    ...
    def write(self, vals):
        ...
        return super(VmSubjectRegistration, self).write(vals)
