...
class ReportFeesAnalysis(models.AbstractModel):
    _name = "report.viseducat_fees.report_fees_analysis"
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if data['fees_filter'] == 'student':
            student_ids = self.env['vm.student'].browse([data['student']])
        else:
            student_ids = self.env['vm.student'].search(
                [('course_detail_ids.course_id', '=', data['course'])])
        docargs = {
            'doc_ids': self.ids,
            'doc_model': 'vm.student',
            'docs': student_ids,
