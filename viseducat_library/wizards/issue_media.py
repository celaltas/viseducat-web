...
class IssueMedia(models.TransientModel):
    ...
    def check_max_issue(self, student_id, library_card_id):
        media_movement_search = self.env["vm.media.movement"].search(
            [('library_card_id', '=', library_card_id),
             ('student_id', '=', student_id),
             ('state', '=', 'issue')])

        if len(media_movement_search) < self.env["vm.library.card"].browse(
        ...
        for media in self:
            ...
            if media.check_max_issue(media.student_id.id, media.library_card_id.id):
                if media.media_unit_id.state and media.media_unit_id.state == 'available':
                    ...
                    self.env['vm.media.movement'].create(media_movement_create)
