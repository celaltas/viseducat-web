...
class TestAssignment(TestAssignmentCommon):
    ...
    def test_case_assignment(self):
        assignment = self.vm_assignment.search([])
        if not assignment:
...
class TestAssignmentSubline(TestAssignmentCommon):
    ...
    def test_case_assignment_subline(self):
        assignment_subline = self.vm_assignment_subline.search([])
        assignment = self.env["vm.assignment"].create({
            'name': "LRTP - 001 - Asg - 009",
            'state': "draft",
            'marks': 50,
            'assignment_type':
                self.env.ref('viseducat_assignment.vm_assignment_1').id,
            'issued_date': time.strftime('%Y-%m-01'),
            'course_id': self.env.ref('viseducat_core.vm_course_4').id,
            'batch_id': self.env.ref('viseducat_core.vm_batch_3').id,
            'subject_id': self.env.ref('viseducat_core.vm_subject_10').id,
            'faculty_id': self.env.ref('viseducat_core.vm_faculty_2').id,
            'submission_date': time.strftime('%Y-%m-01'),
            'allocation_ids': self.env.ref("viseducat_core.vm_student_9"),
            'description': 'Please answer the following questions briefly:'
                           ' - 1. What are the different types of land',
        })
        assignment_subline1 = self.vm_assignment_subline.create({
            'assignment_id': assignment.id,
            'state': "draft",
            'student_id': self.env.ref("viseducat_core.vm_student_9").id,
