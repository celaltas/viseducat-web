...
class GradingAssigment(models.Model):
    ...
    subject_id = fields.Many2one('vm.subject', string='Subject')
...
class vmAssignment(models.Model):
    _name = "vm.assignment"
    ...
    faculty_id = fields.Many2one(
        'vm.faculty', 'Faculty', default=lambda self: self.env[
            'vm.faculty'].search([('user_id', '=', self.env.uid)]),
        required=True)
    ...
    allocation_ids = fields.Many2many('vm.student', string='Allocated To')
    assignment_sub_line = fields.One2many('vm.assignment.sub.line',
                                          'assignment_id', 'Submissions')
    reviewer = fields.Many2one('vm.faculty', 'Reviewer')
    ...
    def onchange_course(self):
        self.batch_id = False
        if self.course_id:
            subject_ids = self.env['vm.course'].search([
