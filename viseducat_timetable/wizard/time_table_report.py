...
class SessionReport(models.TransientModel):
    ...
    def gen_time_table_report(self):
        template = self.env.ref('viseducat_timetable.report_teacher_timetable_generate')
        data = self.read(['start_date', 'end_date', 'course_id', 'batch_id', 'state', 'faculty_id'])[0]
        if data["state"] == 'student':
            time_table_ids = self.env['vm.session'].search(
            ...
            template = self.env.ref('viseducat_timetable.report_student_timetable_generate')
        else:
            teacher_time_table_ids = self.env['vm.session'].search(
                [('start_datetime', '>=', data['start_date']),
                 ('end_datetime', '<=', data['end_date']),
